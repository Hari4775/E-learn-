...
export const setPhone=(phone:string)=>(dispatch:any)=>{
...
export const {setPhoneValue}=PhoneSlice.actions;
...
export const forgotPassword = (body: any): AppThunk => async (dispatch) => {
    dispatch(setLoading(true));
    try {
        let URL = '/register';
        const response = await axiosInstance({ data: { url: URL, method: 'post', data: body }, token: false });
